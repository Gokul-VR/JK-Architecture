        ...
        {projectCategories.map((category, index) => (
          <>
            ...
            <div key={category.id} className="mb-24 container mx-auto mt-20">
              ...
              {/* Grid Section */}
              <div
                className={`grid grid-cols-1 ${category.gridConfig || "md:grid-cols-3"} gap-6 md:gap-10  px-0.5 md:px-[6rem]`}
              >
                {category.projects.map((project, index) => {
                  return (
                    <div
                      key={project.id}
                      className={`group relative overflow-hidden cursor-pointer animate-fade-in mx-auto mt-[1rem] w-full flex flex-col h-full ${project.colSpan || ""}`}
                      style={{ animationDelay: `${index * 100}ms` }}
                      onClick={() => navigate(`/project/${project.id}`)}
                    >
                      <div
                        className={`${project.aspect || "aspect-[6/8]"} max-h-[500px] overflow-hidden transition-all duration-300 flex-1`}
                      >
                        <img
                          src={project.image}
                          alt={project.name}
                      ...
                      <div className="mt-4">
                        <h3 className="text-[1.75rem] font-[500]">
                          {project.name}
                        </h3>
                        <p className="text-[1.2rem] font-poppins font-[300]">
                          {project.location}
                        </p>
                      </div>
                    </div>
